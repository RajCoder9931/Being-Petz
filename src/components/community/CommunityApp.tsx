...
import ChatScreen from "./ChatScreen";
import CreateCommunityForm from "./CreateCommunityForm";
import SuggestedCommunity from "./SuggestedCommunities";
  ...
  return (
    <div className="flex flex-col h-screen pt-14">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <Sidebar
          isOpen={sidebarOpen}
          onClose={() => setSidebarOpen(false)} // only user-triggered
        />
        {/* Main content */}
        <div className="flex-1 flex gap-4 p-4 overflow-hidden">
          {/* Left panel: Community List + Chat */}
          <div className="w-2/3 flex flex-col bg-gray-50 p-4 border-r overflow-y-auto rounded-lg shadow">
            {/* Community List */}
            <div>
              <h2 className="text-xl font-bold mb-4 text-purple-700">My Communities</h2>
              <CommunityList
                communities={communities}
                onCreate={() => setShowForm(true)}
                onSelectCommunity={(c) => setActiveChat(c)}
              />
            ...
            {/* Chat Section below the community list */}
            <div className="flex-1 mt-6 overflow-y-auto bg-white p-4 rounded-lg shadow">
              {activeChat ? (
                <ChatScreen community={activeChat} onBack={() => setActiveChat(null)} />
              ) : (
          ...
          {/* Right panel: Suggested Communities */}
          <div className="w-1/3 p-4">
            <SuggestedCommunity communities={communities} onSelect={setActiveChat} />
          </div>
        </div>
      </div>
    </div>
